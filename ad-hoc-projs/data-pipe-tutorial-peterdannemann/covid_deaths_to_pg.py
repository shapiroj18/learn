...
def load_csv_to_postgres(filename: str, table: str) -> None:
    ...
    try:
        with psycopg2.connect(dbname = os.environ['PGDBNAME'], user = os.environ['PGUSER'], password = os.environ['PGPASSWORD'], host = os.environ['PGHOST']) as conn, \
        ...
        logger.exception('Something went wrong trying to copy file %s to Postgres table %s', filename, table)
        raise
